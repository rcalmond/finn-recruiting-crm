/**
 * sr-inbound.ts — SportsRecruits inbound ingestion.
 *
 * Extracted from the webhook route so that BOTH the live webhook and quarantine
 * REPLAY run this exact code (design Amendment 3: replay uses the identical
 * ingestion path, including the dedup check — never a bypass). A replayed
 * message that also arrived by another route collapses on dedup rather than
 * duplicating.
 *
 * Everything here is family-scoped by its caller: `admin` is a familyAdmin
 * client, `identity` is that family's FamilyIdentity. No module-level identity
 * state exists — see §6 of the email-boundary design.
 */
...
import type { FamilyIdentity } from '@/lib/family-identity'
import { strictCatalogMatchByName, autoAddSchoolFromInbound } from '@/lib/school-auto-add'

/** The SendGrid Inbound Parse fields this pipeline consumes. Stored verbatim on
 *  a quarantine row so a replay reconstructs the identical input. */
...
/** Escape a runtime-built regex fragment (player names, address local parts). */
...
function isSRNotification(subject: string, body: string, playerFirst: string | null): boolean {
  ...
  // §6: the player-name alternative is per-family, never a literal.
  const nameAlt = playerFirst ? `|${escapeRe(playerFirst)}` : ''
  const hasSRSubject = new RegExp(`sent (?:you${nameAlt}) a message`, 'i').test(subject)
...
function extractSchoolAndCoach(
  ...
  const nameAltA = playerFirst ? `|${escapeRe(playerFirst)}` : ''
  const subjectMatchA = outerSubject.match(
    new RegExp(`^(?:Fwd:\\s*)?(.+?)\\s*\\((.+?)\\)\\s+Sent (?:You${nameAltA}) a Message`, 'i')
  )
  if (subjectMatchA) {
 ...
 */
function extractMessageBody(body: string, identity: FamilyIdentity): string {
  ...
  const endPatterns = [
    ...
    ...(identity.playerFirstName ? [`To view ${identity.playerFirstName}'s full profile`] : []),
  ]
  ...
  // §6: quoted-reply detection keys on the FAMILY's own name and sending
  // addresses (local parts), never a hardcoded person.
  const selfTokens = [
    identity.playerName,
    ...Array.from(identity.sendingAddresses).map(a => a.split('@')[0]),
  ].filter((t): t is string => !!t && t.length > 1).map(escapeRe)
  const quoteMatch = selfTokens.length > 0
    ? extracted.match(new RegExp(`\\nOn\\s+[A-Z][a-z]{2}\\s+\\d{1,2},\\s+\\d{4}.*(?:${selfTokens.join('|')})`, 'i'))
    : null
...
function formatDateForParser(d: Date, timeZone: string): string {
...
async function handleOutboundCC(
  ...
  const dateStr    = formatDateForParser(emailDate, identity.homeTimezone)
  ...
  const messages    = parseSRPaste(pasteBlock, true, identity.homeTimezone) // debug=true → logs to console.error
...
// ── Ingestion ─────────────────────────────────────────────────────────────────
//
// The post-routing pipeline, unchanged in behaviour from the single-family
// version: SPF/DKIM → forward unwrap → outbound-CC intercept → SR drops →
// school/coach match → dedup → insert → fire-and-forget cascade.

export async function ingestSrMessage(
  admin: Supabase,
  identity: FamilyIdentity,
  fields: InboundFields,
  receivedAt: string,
): Promise<IngestResult> {
  const { from, subject, spf, dkim, headers } = fields
  const rawText = fields.text
  const rawHtml = fields.html
  ...
  if (isOutboundCC(subject, innerBody)) {
    ...
    await handleOutboundCC(admin, identity, subject, innerBody, headers, fullBodyText, sourceMessageId, receivedAt)
    return { status: 'ok' }
  }
  ...
  const { schoolName: parsedSchoolName, coachName: parsedCoachName, srSubject } =
    extractSchoolAndCoach(subject, innerBody, identity.playerFirstName)
  const messageBody = extractMessageBody(innerBody, identity)
  ...
  const notes: string[] = []
  let schoolId: string | null = null
  let autoAddedSchoolId: string | null = null
  ...
  if (!parsedSchoolName) {
    ...
    if (school) {
      ...
      // AUTO-ADD: a coach reaching out IS engagement. SportsRecruits asserts the
      // coach's school structurally ("Coach (School) just sent a message"), which
      // is HIGH-confidence evidence — but the catalog resolution is strict
      // (exact name/short_name, exactly-one-or-refuse), never the loose
      // substring rules and never a subject-word guess.
      const catalog = await strictCatalogMatchByName(parsedSchoolName)
      if (catalog) {
        const evidence =
          `auto-added from inbound SportsRecruits mail: SR named "${parsedSchoolName}"` +
          `, matched catalog row "${catalog.name}" (${receivedAt.slice(0, 10)})`
        const added = await autoAddSchoolFromInbound(admin, catalog, evidence, null)
        if (added.schoolId) {
          schoolId = added.schoolId
          autoAddedSchoolId = added.schoolId
          notes.push(`School auto-added at C-tier from the catalog: "${catalog.name}"`)
        } else {
          notes.push(`No school match for parsed name "${parsedSchoolName}"; auto-add refused (${added.refusedBecause})`)
          parseStatus = 'orphan'
        }
      } else {
  ...
  if (insertError) {
    ...
    // Swallow — non-2xx would trigger SendGrid retries into the same failure
    return { status: 'ok' }
  }
  ...
  // Provenance: point the auto-added school at the message that justified it,
  // so /unmatched can explain and undo the add.
  if (insertedRow?.id && autoAddedSchoolId) {
    void admin.from('schools')
      .update({ origin_contact_log_id: insertedRow.id })
      .eq('id', autoAddedSchoolId)
      .then(({ error }) => {
        if (error) console.error('[auto-add] provenance backfill failed:', error.message)
      })
  }
