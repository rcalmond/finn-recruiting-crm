...
Return ONLY the JSON object. No commentary before or after.`
  ...
  if (coaches.length > 0) {
    ...
    for (const c of coaches) {
      const flags = [c.isPrimary ? 'primary' : null, c.hidden ? 'hidden by the family — do not recommend contacting' : null, c.needs_review ? 'needs_review' : null].filter(Boolean).join(', ')
