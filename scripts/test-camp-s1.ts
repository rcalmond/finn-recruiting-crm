...
async function runOne(db: any, anthropic: Anthropic, schoolQuery: string, player: DocPlayerProfile, schoolList: DocSchoolListItem[]) {
  ...
  const userPrompt = buildCampDocUserPrompt({
    ...
    research: research?.snapshot ?? null, researchStatus: research?.status ?? null, schoolName: school.name, schoolList,
  })
...
async function main() {
  ...
  await runOne(db, anthropic, 'middlebury', player, schoolList)
  await runOne(db, anthropic, 'colby', player, schoolList)
}
