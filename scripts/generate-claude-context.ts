...
|---|---|---|
| 2026-08-07 | Get Ready buildout: Assets rebuilt as visual cards (reel with ghost play-triangle anchor, document cards, NEW Test Scores card showing actual SAT/AP numbers read from a structured player_scores block on player_profile — migration 060, not hardcoded). School Discovery v1: discovery_schools universe (migration 059, 809 curated men's-soccer programs across D1/D2/D3/NAIA/JUCO with division/region/academic/enrollment facets; NY→Northeast convention), facet browse + add-to-list into the working pipeline (C-tier, Not Contacted, facets folded into notes), and LLM "find more like these" (/api/discover/similar, Sonnet-class) — 3+ seed schools produce 5–8 reasoned lookalikes, token-matched to the universe so facets ride along, seed-excluded, cached per seed-set hash with a Refresh override. Get Ready next-move card points at live Discover when assets are fresh. | Feature + Schema |
