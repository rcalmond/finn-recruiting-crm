...
- A DISTINCT OVER ROWS ENUMERATES WHAT EXISTS, NEVER WHAT IS PERMITTED, AND THE TWO DIVERGE SILENTLY. Same family as the grep rule, one level down: a grep enumerates the hypothesis rather than the population, and a value distribution enumerates the population rather than the DOMAIN. a select of role from coaches returned five values and was reported as though it were the vocabulary; the CHECK constraint permits six, and the missing one — 'Interim Assistant Coach' — has zero rows, so no query over data could ever have shown it. The conclusion drawn (that Goalkeeper Coach is absent) happened to be right, by the method that had already produced a 16-of-40 inventory. WHEN THE QUESTION IS WHAT IS ALLOWED, ASK THE CONSTRAINT, THE ENUM OR THE TYPE — never the rows. The inverse also bites: a UI's allowed-values list can drift ABOVE the constraint (api/gmail-partials/[id] offers three roles the CHECK rejects), and no row will ever reveal that either.
- A RULING THAT CONTRADICTS A VERIFIED RECON MUST BE RECONCILED BEFORE IT BECOMES SQL. Chunk F1 v1 aborted on a column that did not exist, because a ruling asserted schools.primary_coach_id was already present and set on 30 schools while the recon had verified, with output, that it did not exist anywhere in the schema or the codebase. The recon was right and had said so precisely; the ruling was emitted to the architect chat anyway. THE ABORT WAS THE GOOD OUTCOME — it failed loudly at the DDL. The reconciliation step is cheap and belongs BEFORE the SQL: when a decision and a verified finding disagree, one of them is wrong and finding out which costs one query.
