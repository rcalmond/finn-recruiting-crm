...
**T1 family scoping:** all 24 pre-existing family tables carry family_id NOT NULL + FK → families + index, protected by family RLS (see below); the new players table is family-scoped from birth. Renames live since the C6 sitting: camp_finn_status → camp_family_status (composite unique camp_id + family_id), school_message_plan.finn_notes → family_notes, get_voice_references() → get_voice_references(p_family_id uuid). Summary/message-plan upserts target the composite (school_id, family_id) keys; camp-status upserts target (camp_id, family_id). SUPERSEDED BY T2 (2026-08-18): schools, coaches, camps, camp_school_attendees and camp_coach_attendees are FAMILY tables carrying family_id — they are NOT catalog. The tables that remain shared catalog are discovery_schools, school_research, camp_proposals, coach_changes, cron_runs, not_found_log, families and users (see Tenancy Architecture in Section 9).
...
Two migrations have landed whose CODE has not. Reading the schema and assuming the code matches would reintroduce a defect that was deliberately closed. Both states verified from the code on 2026-08-19.

1. CHUNK I. camp_proposal_decisions EXISTS (195 rows — every historical Almond rejection backfilled as an Almond-scoped dismissal) and camp_proposals.status accepts invalid. THE CODE IS UNTOUCHED: shouldSkipProposal in src/lib/camp-extractor.ts still carries its OLD signature with no familyId argument, camp_proposal_decisions has ZERO references anywhere in src/, and the pending-decision rule is not implemented. CONSEQUENCE: the camp-discovery cron's ALMOND_FAMILY_ID pin is STILL the only thing preventing one family's rejection from silently suppressing a camp for every other family. DO NOT REMOVE THE PIN, and DO NOT enable any union-of-families scan set, until the signature change and the pending rule are in and verified. The 195 dismissals are inert until code reads them, and the pin is a control only by accident.
2. ADMIN AUDIT COLUMNS. inbound_quarantine.resolved_by and resolved_by_email, plus family_inbound_addresses.minted_by and minted_by_email, all EXIST. NONE of the four write paths populates them. Path by path: requireAdmin in src/lib/admin-gate.ts returns ok and userId only and never returns an email, so no path has an email to write; the quarantine REPLAY and DISCARD branches in /api/admin/quarantine/[id] write status, resolved_at, resolved_family_id and resolver_note but neither resolved_by field; mintInboundAddress inserts family_id, address, label and status only, and its signature takes no minter argument at all. Every existing row reads null. Three of the four paths already hold the admin user id in scope from the gate and need only two more fields written; the email requires changing what the gate RETURNS; mint requires a signature change. UNTIL THEN THE COLUMNS ARE DECORATION — do not read them as an audit trail and do not report replay or mint provenance from them.
...
- E1 LINKAGE STATE. Almond has 65 schools, 33 linked to the catalog; Testerson has 13, all 13 linked. The nameKey bridge splits Almond's remaining 32 into 19 auto, 2 ambiguous, 11 unmatched. SJSU is verified GENUINELY ABSENT from the catalog (full paginated 1066-row scan, diacritic-folded — the catalog contains no accented characters at all), so it stays unlinked with a recorded reason rather than getting an invented row. Washington University resolves to Washington U. St. Louis on CAMP-NAME evidence, not on name matching.
