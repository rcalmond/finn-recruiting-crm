...
- E1 OPEN QUESTION — EVERY UNLINKED SCHOOL CARRIES AN ASSERTED DIVISION. schools.division was NOT NULL with a CHECK until 2026-08-20, so every school ever added had to be given one of the five values whether or not anyone knew it. That is exactly why the old off-universe add wrote a fabricated D3. 32 of Almond's schools are unlinked, and some of their divisions may be as invented as that D3 was. DO NOT investigate separately: E1's linkage pass resolves each of them to a catalog row, and the catalog row supplies the truth, so the answer falls out of work already planned. Check the linked division against the stored one during E1 and treat a disagreement as evidence about the stored value, not the catalog.
- CATALOG PROPOSAL REVIEW HAS NO MANUAL MERGE TARGET. The review screen only offers merge into rows the MATCHER found. When the matcher returns nothing but the reviewer knows the school is in the catalog — the false-negative case the merge path exists for, e.g. University of Wisconsin Madison, which never reaches Wisconsin — there is no way to merge from that screen; the only options are create or reject. A reviewer search box on the review row would close it. Until then a reviewer facing that case should reject with a note rather than create a duplicate.
