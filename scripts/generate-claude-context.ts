...
- E1 ASSERTED-DIVISION QUESTION — ANSWERED 2026-08-20, and the answer is reassuring. schools.division was NOT NULL with a CHECK until that day, so every school ever added had to be given one of five values whether or not anyone knew it, which is why the old off-universe add fabricated a D3. Evidence collected free during E1's linkage pass: across ALL 58 of Almond's schools now linked to a catalog row, the stored division matches the catalog's in EVERY case — ZERO disagreements. The invented-division risk did not materialise in real data. CAVEAT, and it is the honest limit of the evidence: the 7 rows the matcher still cannot resolve carry stored divisions nobody has checked against anything, so the finding covers what is checkable, not everything.
