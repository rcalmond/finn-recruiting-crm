...
- A GUARD'S EXCLUSIONS ARE PART OF ITS CLAIM, and its heuristics are too. The embed gate skipped any select matching /^(count|id)\b/ without an alias — meant to ignore count shorthands, it actually dropped every select that merely STARTED with "id," and used an UNALIASED embed. FIVE contracts were being silently skipped, including two in PRODUCTION: api/gmail-partials (the review queue) and camp-extractor (the camp-discovery cron), both returning PGRST201 at runtime since the FK landed. The gate reported "12 embeds verified" the whole time — a true statement that answered a narrower question than anyone reading it believed. Its matcher also allowed only ONE !hint, so schools!constraint!inner was invisible to it. WHEN A GATE REPORTS A COUNT, ASK WHAT IT DECLINED TO COUNT.
