...
- A ONE-FAMILY TEST SET CANNOT FIND A BUG WHOSE TRIGGER IS STATE THE SECOND FAMILY DOES NOT HAVE YET. Every camp_family_status row in the database was seeded by createCamp for the family that created the camp, so Almond had 76 of 76 and its UPDATE branch always matched — the INSERT path had never executed for anybody in a year of use. Testerson starting EMPTY exposed two defects within minutes of each other: the missing insert (a bare UPDATE matching zero rows, returning no error, doing nothing) and then a DOUBLE-FIRE race in the select-then-insert replacement, which only shows on a family's very FIRST status because once the row exists both writes are updates and neither collides. THE GENERAL SHAPE: absence is the second family's normal starting state and the first family's impossible one, so every code path gated on "does this row exist" is untested by the founding tenant. When a table is the per-family layer of a shared parent, THE EMPTY CASE IS THE COMMON CASE for everyone who arrives later — test it first, not last.
