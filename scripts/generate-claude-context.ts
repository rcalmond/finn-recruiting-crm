...
// T2 Shape B: raw service reads BYPASS RLS — every family-table query below
// must scope explicitly or the pipeline section mixes families.
const ALMOND_FAMILY_ID = '00000000-0000-0000-0000-000000000001'
...
function parseExistingFile(filePath: string): { header: string; footer: string } | null {
  ...
  //
  // THE TRAILING SEPARATOR IS STRIPPED, and that fix is the whole reason this
  // comment exists. main() writes `footer + '---\n\n' + section15`, so the
  // separator belongs to the GENERATOR — but the preserved footer slice runs up
  // to the Section 15 heading and therefore INCLUDED the separator the previous
  // run had just written. Every export re-preserved it and added another, so the
  // rules accumulated one per run: the doc had reached TWENTY consecutive
  // horizontal rules before Section 15 by 2026-08-22. Nothing failed, nothing
  // reported it, and the count grew on every regeneration — the same shape as
  // every other silent-growth defect on this project. Found by diffing the
  // round-trip rather than by reading the output.
  const s15Idx = content.indexOf(SECTION_15_MARKER)
  const footerEnd = s15Idx === -1 ? content.length : s15Idx
  const footer =
    '\n---\n\n' +
    content
      .slice(s12Idx, footerEnd)
      .replace(/(?:\s*\n---)+\s*$/, '')   // drop the separator run this file already carries
      .replace(/\n*$/, '\n\n')
...
STILL A FAMILY TABLE, and E2 chunk G is the sitting that changes that. Read the E2 — Catalog Coaches section in Section 9 before touching it.

\`\`\`
id                  uuid PK
family_id           uuid FK → families.id     -- T2 Shape B; DEFAULT app.current_family_id()
school_id           uuid FK → schools.id (cascade delete)   -- G DROPS this
discovery_school_id uuid FK → discovery_schools.id (NO ACTION)  -- E2 chunk F; 254/254 backfilled, zero nulls
name                text
role                text                   -- the EIGHT values of COACH_ROLES (chunk 5b) — see src/lib/coach-roles.ts
email               text
is_primary          boolean                -- LEGACY. schools.primary_coach_id wins; G DROPS this
is_active           boolean                -- ROSTER TRUTH: false = the scraper says they left the program
needs_review        boolean                -- true = flagged for human review (coach_departed applies this)
sort_order          integer
notes               text                   -- G DROPS this; the private layer owns family notes
role_source         text                   -- chunk F: the RAW roster string, before normalizeRole
endowed_title       text                   -- chunk F: e.g. Bobby Clark, separate from role
archived_at         timestamptz            -- LEGACY hide; the private layer owns hidden_at now
source              text not null          -- 'manual' (default) | 'scraped' (roster scraper) | 'from_gmail' (Gmail partials UI)
created_at          timestamptz
updated_at          timestamptz
\`\`\`

HOW THIS LIST WAS OBTAINED, because the rule about migration files applies here too: assembled from src/lib/types.ts (the Coach interface), chunk F's confirmed SQL, and Section 15's live FK rows — NOT from a catalog dump. src/lib/types.ts has NOT been widened for F's three columns; only coach-scraper's own DbCoach carries them. Ask the catalog before treating this block as complete.

### Table: \`coach_family_state\` (E2 chunk F1, architect-chat SQL — not in supabase/migrations/)
\`\`\`
coach_id   uuid FK → coaches.id (cascade delete)
family_id  uuid FK → families.id   -- DEFAULT app.current_family_id()
hidden_at  timestamptz             -- set = THIS FAMILY does not want to see them
notes      text                    -- this family's own note about this coach
updated_at timestamptz
\`\`\`
THE PRIVATE LAYER. Sparse BY DESIGN — absence is the default, so no row means not hidden and no note. hidden_at is a FAMILY PREFERENCE; coaches.is_active is ROSTER TRUTH; conflating them is what the table exists to end. Un-hiding a row that carries no note DELETES the row rather than writing hidden_at null into it (chunk F). Columns read from src/lib/coach-family-state.ts and Section 15's FK rows.
...
\`\`\`
coach_page_url              text      -- URL of the school's official MEN'S soccer coaches page.
                            -- ONE URL PER SCHOOL is the assumption, and it is about to be wrong:
                            -- staff pages are PER PROGRAM. See Women's Soccer in Section 9.
...
THE FENCE IS BLIND TO SQL FUNCTION GRANTS, and this is the gap that matters most about it. scripts/check-tenancy-fence.mjs checks how src/ constructs a service-role CLIENT; it knows nothing about who may EXECUTE a database function. A SECURITY DEFINER function that takes a family id as an ARGUMENT is a tenancy boundary living entirely outside the fence's reach — it must be service_role-only or derive and check the caller's family internally, and neither condition is verifiable from the repo. public.get_voice_references was exactly this: SECURITY DEFINER, executable by ANON, taking p_family_id with no caller check, so anyone holding the anon key — which ships in the client bundle BY DESIGN — could call it unauthenticated with any family uuid and read 15 real outbound message summaries with school and coach names. The family ids are sequential and guessable (Almond ...0001, Testerson ...0002). SAY THIS PART PLAINLY: it was found by a sweep prompted by an UNRELATED over-grant on a different function, not by any guard we have. No gate in this project could have caught it, and none can catch the next one. Functions cannot move to the app schema, where the default-privileges grant does not reach, because PostgREST only exposes public — which is precisely why app.current_family_id() was never at risk and this was.

CORRECTED 2026-08-22 — this paragraph previously ended by prescribing an internal caller assertion as the second lock, and that prescription was SUPERSEDED the same day. The residual risk it names is real (create or replace preserves grants, drop-and-recreate does not, and the architect chat re-emits functions), but an assertion is still a lock a re-emission can drop. THE RULING IS PREFER SECURITY INVOKER OVER A DEFINER WITH AN ASSERTION — see Tenancy Process Rules in Section 9 for the reasoning and the current per-function catalog. get_voice_references is now INVOKER, so family RLS on contact_log is the lock and it holds by construction.
...
ONE AUDIT FINDING IS CLOSED, and it is called out because the dated changelog row that reported it still reads as an open gap. The 2026-08-22 audit found assertFixtureShape validating ONLY the coaches array — a guard scoped to the thing that had just broken, reading as though it validated the fixture. It was widened the same day and the description above is the CURRENT behaviour: thirteen top-level keys, player.home_timezone, and the contactLog / offers / coaches row shapes. Everything else in this section is still true.

### What the three guards do NOT check (August 2026 — description, not history)

Audited read-only on 2026-08-22 and recorded here rather than only in a changelog row, because a future session needs this as DESCRIPTION. WHEN A GATE REPORTS A COUNT, ASK WHAT IT DECLINED TO COUNT.

THE TENANCY FENCE — scripts/check-tenancy-fence.mjs:
- IT SCANS src/ ONLY. CORRECTED 2026-08-22: the New Binding Rules entry below says all three guards now scan scripts/, and for this one that is WRONG — the file calls walk('src') and nothing else, and its own success line says in src/. The typechecker and the embed gate were both extended; the fence was not. THE OBSTACLE IS REAL, not an oversight to fix casually: ten scripts legitimately reference SUPABASE_SERVICE_ROLE_KEY, including generate-claude-context.ts and the camp-doc harness, so extending the fence means designing an allowlist for the scripts that are SUPPOSED to hold service role, and that is a decision, not a one-line change.
- It matches .ts and .tsx ONLY, so the five .jsx files under design-preview are invisible to it. None touches Supabase today; that was verified, not assumed.
- It detects exactly TWO patterns: the literal SUPABASE_SERVICE_ROLE_KEY, and a NAMED import of createClient from the supabase-js package. A namespace import, a require, or a dynamic import would all pass it silently.
- IT CHECKS CONSTRUCTION, NEVER USE. Nothing enforces the doc's own claim that rawService is storage, auth and rpc plumbing only. THIS IS THE GAP CHUNK G WALKS INTO — see E2 — Catalog Coaches.
- It cannot see .rpc() calls at all, which is the exact shape the get_voice_references exposure lived in.

THE EMBED-CONTRACT GATE — scripts/check-embed-contracts.mjs: its count/id heuristic was removed on 2026-08-22 after it silently skipped five contracts, two of them live production paths; verified embeds went 12 to 17. It still only sees embeds it can extract statically from a select string.

THE CAMP-DOC HARNESS: not a gate at all. See the section immediately above.
...
4. THE ROUTES — ~25 user-facing routes/pages run on the RLS-enforcing user client via getFamilyContext (src/lib/require-family.ts). SSE/LLM generators, triage routes, bulk-import, and gmail flows keep service role via familyAdmin(familyId). The sendgrid inbound webhook is now PER-FAMILY: it resolves the family from the envelope and runs familyAdmin(familyId). CORRECTED 2026-08-22 — this item previously said ALL FOUR content crons run familyAdmin(ALMOND_FAMILY_ID) and that the camp-discovery pin was load-bearing and must not be removed. BOTH CLAIMS WERE STALE and the same section's SCHEMA AHEAD OF CODE item already contradicted them. THE PIN CAME OFF camp-discovery AND coach-roster-sync on 2026-08-19: both now assemble a UNION of families through src/lib/cron-scan-set.ts, family by family via scoped clients, so every write lands in the scope that produced it. Verified in code, not inferred — the pin constant appears in those two routes only as a comment recording its removal. STILL PINNED, and correctly: gmail-sync and summary-refresh, both bound to Almond's single Gmail OAuth until per-family OAuth exists. NONE of the four runs catalogAdmin. Only quarantine-reaper uses rawService. get_voice_references is called with p_family_id via the wrapper's introspectable scope (scopeOf).
...
- PER-FAMILY EMAIL ROUTING SHIPPED (see Email Boundary — Current State). What remains Almond-hardcoded is TWO content crons, gmail-sync and summary-refresh, both bound to Almond's single Gmail OAuth. CORRECTED 2026-08-22: this line used to say all four and to forbid removing the camp-discovery pin; that pin came off on 2026-08-19 along with coach-roster-sync's.

### Tenancy Process Rules (August 2026 — permanent, binding)

Forged during T1; they bind every future DB-touching stretch:

- A FUNCTION EMITTED IN public IS EXECUTABLE BY anon AND authenticated BY DEFAULT. LOCKING ONE NEEDS THREE REVOKES — from PUBLIC, from anon, AND from authenticated — and each alone reads as done while leaving a hole. Learned twice in one hour: revoking PUBLIC alone missed fk_graph's DIRECT grants (Supabase's ALTER DEFAULT PRIVILEGES hands EXECUTE straight to anon and authenticated on every new public-schema function), and revoking the direct grants alone missed set_current_research's INHERITED grant through PUBLIC membership — which has_function_privilege reports as true. PROVE IT WITH has_function_privilege, for all three roles; that check is the only reason either miss was visible. Supabase ships ALTER DEFAULT PRIVILEGES granting EXECUTE on new public-schema functions DIRECTLY to anon and authenticated, and a revoke aimed at the PUBLIC role does not remove a direct grant to a named one — so the revoke RUNS, REPORTS SUCCESS, and leaves the function open. Caught on public.fk_graph() (2026-08-22), where any signed-in browser session could have read the entire schema graph. app-schema functions are UNAFFECTED, which is why app.current_family_id()'s service_role restriction held: the default-privileges grant targets public only. THE VERIFICATION IS THE OUTPUT, NOT THE REVOKE — the same shape as every other rule on this list, and the reason this one is here rather than assumed.
PREFER SECURITY INVOKER OVER A DEFINER WITH AN ASSERTION. get_voice_references was SECURITY DEFINER taking p_family_id, and the proposed fix was an internal caller check so the grant stopped being the only lock. THE BETTER FIX WAS TO DROP DEFINER ENTIRELY: service_role has BYPASSRLS so the one legitimate caller works unchanged, while any user session inherits family RLS on contact_log and gets ZERO ROWS even when handed another family's id. That closes the boundary BY CONSTRUCTION rather than by a grant or an assertion — and it matters because a grant does not survive DROP+CREATE and this project re-emits functions routinely, so an assertion is still a lock a re-emission can drop. RLS is not. The general form: when a function's safety can be expressed as a policy the database already enforces, express it there, not as a check the next emission might forget.

EVERY FUNCTION IN public, from the catalog (2026-08-22) — recorded so nobody "fixes" the last two without knowing why:
  fk_graph                          DEFINER  service_role only
  get_voice_references              invoker  service_role only
  set_current_research              invoker  service_role only
  set_updated_at                    invoker  OPEN — trigger function; raises if called directly
  update_player_profile_updated_at  invoker  OPEN — same
...
THE POLICY IS SWAPPED AND CAMPS IS LOCKED — 2026-08-22, and this paragraph exists because the doc previously recorded only the CODE half and a reader could have concluded the database hole was still open. It is not. camps and camp_school_attendees carried ALL/true/true RLS, so any signed-in browser session could insert, update or delete a shared camp directly, with the route layer offering no protection whatever. The policy was replaced at the architect sitting and all four checks passed. THREE PARTS HAD TO LAND TOGETHER, and the order matters for anyone reading this as precedent for chunk G:
1. THE ENDPOINTS FIRST, as a deploy — five admin routes (POST /api/camps, PATCH and DELETE /api/camps/[id], POST and DELETE /api/camps/[id]/attendees), each requireAdmin plus catalogAdmin.
2. THE UI GATE IN THE SAME COMMIT, never as a follow-up. For a non-admin family every camp field renders READ-ONLY; camp facts ARE shared truth, so seeing them without editing them is honest and needs no error copy.
3. THE POLICY SWAP LAST, once nothing in the browser still wrote the table.
That is WIDEN BEFORE, NARROW AFTER applied to a policy instead of a CHECK: narrowing what the database accepts while the deployed client still writes it is the same defect in a different costume. camp_family_status was deliberately left alone — it is the FAMILY layer, still written from the browser, still RLS-scoped to the caller, which is why createCamp and updateCamp are two halves on two clients.
...
### E2 — Catalog Coaches (August 2026, current state — G IS NEXT AND G IS IRREVERSIBLE)

Coaches move from per-family rows to the shared catalog keyed on discovery_schools, with a per-family private layer for family posture. Same split as E1.5: who is on a staff is a FACT ABOUT THE WORLD; who a family talks to, who they have hidden, and what they have written is not.

WHAT HAS SHIPPED AND BEEN VERIFIED. Each of these is live in production and none of it is reversible only by SQL — the code halves roll back by reverting a commit.

- CHUNK 2 — READS ACCEPT BOTH PRIMARY DOMAINS. src/lib/coach-primary.ts is the one place that knows who a family talks to: schools.primary_coach_id wins, coaches.is_primary fills in where the pointer is null, NO migration flag is consulted. Writes set both, pointer first.
- CHUNK 3 — THE PRIVATE LAYER IS READ. coach_family_state is wired. Hidden means excluded from pickers and the primary rotation, INCLUDED in generators carrying the flag, and shown collapsed as Hidden by you (n).
- CHUNK 4 — THE 18 INVISIBLE COACHES ARE VISIBLE. Three-way partition: active roster, hidden by you, no longer at this program. Departed reads from is_active alone.
- CHUNK 5a — ONE DEFINITION OF A COACH ROLE. src/lib/coach-roles.ts owns the vocabulary that had lived in eight places.
- CHUNK 5b — THE EIGHT VALUES AND THE ABBREVIATION FIX. normalizeRole is punctuation-stripped and prefix-matched, most-specific-first.
- CHUNK F — PROVENANCE. SQL added coaches.discovery_school_id (254 of 254 backfilled, zero nulls), role_source and endowed_title; the code half persists them on insert, carries the raw string through coach_changes.details, and refreshes on drift. unhideCoach DELETES an empty coach_family_state row. F IS PURELY ADDITIVE — rollback is three dropped columns.

G IS THE ONLY IRREVERSIBLE STEP IN E2, and it is the next sitting. It re-points coaches onto discovery_schools and then DROPS family_id, school_id, is_primary and notes. After those drops the per-family shape is gone from the row and no code revert brings it back; the only path back is the pre-image tables. Everything before G was additive or code-only. TREAT G AS A DIFFERENT KIND OF SITTING THAN THE FIVE THAT PRECEDED IT.

WHAT G CARRIES:
- THE PRE-IMAGES, CAPTURED IN G'S OWN SITTING — coaches_repoint_preimage (254 rows) and coach_changes_repoint_preimage (299 rows), with the counts asserted IN THE SAME TRANSACTION as the re-point. NOT CAPTURED AHEAD OF TIME, and this corrects F's original runsheet: coach-roster-sync runs Sunday and Wednesday, so a snapshot taken at F and re-pointed days later leaves everything the scraper added in between with no rollback at all. E1.5 recorded this exact lesson for camps and camp-discovery's daily run; E2 initially ignored it and then corrected.
- The Marlon McKenzie duplicate merge, before the unique index can exist.
- The FK to discovery_schools moved to ON DELETE RESTRICT, matching E1.5's three.
- A unique index on the catalog key. IT MUST INCLUDE PROGRAM — see the next section.
- Three indexes recreated on discovery_school_id; idx_coaches_family dropped.
- The RLS policy swap, following the camps precedent recorded above — endpoints and UI gate first, policy last. coach_changes takes a NARROWER shape than camps did.
- The drops: family_id, school_id, is_primary, notes.

BEFORE G'S SQL, THREE THINGS. (1) THE CLOSURE IS PASTED BEFORE THE SQL — Section 15's cascade closure for coaches and coach_changes goes into the sitting before any DDL. (2) The two browser write paths move: updateCoach becomes PROPOSE-ONLY, and insertCoach becomes a shared write plus a proposal with needs_review true, with promoteDomainToCatalog gated on source manual AND needs_review. (3) The coaches narrow policy ships with its UI gate in the same commit.

G'S DESIGN ADDITIONALLY CARRIES TWO ITEMS DECIDED 2026-08-22:

- THE COACH-REMOVAL STORY. As an admin I can remove a coach who should not exist. Today there is NO removal path in the product at all — rejecting a coach_added proposal leaves the row that proposal created, and the only way to delete a coach is SQL. This is not hypothetical: Tim Howard, Goalkeeper Coach at Bowdoin, was created by hand on 2026-08-22 to exercise the hide path and is still there. Once coaches is CATALOG that row is every family's row, so a test fixture becomes shared staff. The shape follows the rules already on this page: removal is a claim about the world, so ACCEPT IS ADMIN-ONLY applies, and a family who merely does not want to see someone already has hidden_at.
- THE FENCE'S CONSTRUCTION-NOT-USE GAP. G moves coaches into CATALOG_TABLES, after which catalogAdmin passes coach reads and writes through UNSCOPED by design. The tenancy fence only ever asked where a client came from, so it will be perfectly happy with a catalogAdmin coach write that should have been family-scoped. THE RESOLUTION IS THE COACH-WRITE INVENTORY, and IT HAS NOT BEEN DONE: enumerate every coach write, decide which are legitimately catalog-wide (the scraper, coach_changes apply) and which must stay family-scoped, and only then decide whether anything needs building. Note that useRealtimeData writes through the BROWSER client and never touches tenant-db at all, so the fence was never going to see those either way. Do not start G's SQL treating this as answered.

AFTER G, IN ORDER. H — the catalog scraper, built PROGRAM-AWARE from the start (its scan set becomes school-and-program pairs, not schools). Then DOMAINS promotion. Then E2.5, the school_research re-key. Deferred and logged, not forgotten: the /schools modal catalog routing (designed, not built); reclassifying the seven Other coaches by targeted scrape AFTER the re-point so the answers land on catalog rows; and promoteDomainToCatalog becoming a proposal in E4.

### Women's Soccer Support — the program dimension (DECIDED 2026-08-22, binding on G)

NEW REQUIREMENT, decided the day before G. It is here because it changes G's PERMANENT KEYS, and a key is the one thing that is expensive to revisit: adding program now is one migration, adding it after G is a re-migration of the table G just re-pointed.

THE DATA MODEL, and the line is between institution and program:

- discovery_schools stays THE INSTITUTION. Bowdoin is one row. It does not fork.
- A PROGRAM DIMENSION — men's soccer or women's soccer — goes on COACHES, on CAMPS, and on COACH-PAGE URLS. That is the honest shape because staff pages, rosters and camps are all PER PROGRAM in the real world: a school has two staffs, two rosters, and runs camps for each.
- G'S PERMANENT KEYS INCLUDE PROGRAM. The catalog uniqueness for a coach is the institution AND the program AND the name, not the institution and the name — two programs at one school can and do employ different people with the same name, and more to the point a men's assistant and a women's assistant are simply not the same row.
- H'S SCAN SET BECOMES SCHOOL-AND-PROGRAM PAIRS. This is why H is built program-aware from the start rather than retrofitted, and it is the same grain lesson src/lib/scan-budget.ts already carries: a unit is an opaque item, so changing the grain is a change at the call site.
- THE PLAYER'S SPORT SCOPES EVERYTHING A FAMILY SEES. A family recruiting a girl sees women's staffs, women's camps, women's coach pages. The player's sport is the filter, applied once, everywhere.

WHAT IS NOT IN THIS DECISION, stated so nobody builds it early:

- WOMEN'S URL DISCOVERY AND SCRAPING ARE PHASE 1.5, AFTER H. The schema carrying a program column is not the same thing as the catalog holding women's data, and the gap between those two is where the honest failure lives. See A FABRICATED DEFAULT IS A CLAIM: an empty women's catalog must render as empty, never as a men's staff wearing a women's label.
- ALPHA WAVE TWO IS GATED ON COVERAGE, NOT ON SCHEMA. Families recruiting on the girls' side are invited when women's coverage is REAL — when the catalog actually holds their programs and their staffs — not when the column exists. Inviting on schema is inviting a family to an empty product and calling it support.
- CATALOG COMPLETENESS WAS ALREADY AN OPEN E2 BLOCKER for the men's side (D2, NAIA and JUCO gaps, and SJSU verified genuinely absent from a near-complete D1 set). Women's coverage is a second axis of the same question, not a separate one.

### New Binding Rules (August 2026 — permanent, binding)

Each earned this week; the incident that produced it is named.

- A FIX FOR A SILENT FAILURE MUST NOT INTRODUCE ONE. It happened TWICE in one afternoon. (1) Closing a browser-writable RLS hole, the new admin endpoints used rawService() for cross-family reads — which the wrapper REFUSES — so add-attendee and delete-camp returned 500 and the UI said nothing, replacing an over-permissive write with an invisible failure. (2) Fixing THAT, an in-flight guard was added to stop a duplicate-key race; it dropped any second camp-status click while one was pending, RETURNED NULL, and said nothing — so the user's click vanished. The guard was also unnecessary: the upsert it shipped alongside had already made two racing writes benign, so it was a lock added after the thing it locked became safe. THE TEST FOR ANY FIX IN THIS FAMILY: if the new code can decline to do what the user asked, can the user tell? If the answer is no, the fix has moved the silence rather than removed it. And check whether the earlier half of the same fix already solved the problem — a second mechanism guarding a solved problem buys nothing and costs a new failure mode.
- ABSENCE IS THE SECOND FAMILY'S NORMAL STARTING STATE AND THE FOUNDING TENANT'S IMPOSSIBLE ONE. Every camp_family_status row was seeded by createCamp for the family that created the camp, so Almond held 76 of 76 and its UPDATE branch always matched — the INSERT path had never executed for anybody in a year of production use. Testerson starting empty exposed two defects within minutes: a bare UPDATE matching zero rows (no error, no write, no message) and then a race in its replacement. THIS IS NOT A CAMPS OBSERVATION. It applies to EVERY table that is the per-family layer of a shared parent — camp_family_status, coach_family_state, camp_proposal_decisions, and whatever E2 adds next — and it PREDICTS WHERE THE NEXT ONE IS: any code path gated on "does this row exist" is untested by the family that has been using the product for a year, because for them it always does. Test the empty case FIRST, and treat a one-family test set as evidence about one family.
- WHEN A CONSTRAINT AND ITS PICKER CHANGE TOGETHER, THE CONSTRAINT MUST NOT SHIP FIRST IF THE PICKER STILL OFFERS THE OLD DOMAIN. WIDEN BEFORE, NARROW AFTER — and the asymmetry is the whole rule. Widening a CHECK is safe ahead of the code because the picker offers a subset of what the database accepts; NARROWING it is not, because the deployed picker still offers a value every insert will now reject. E2 shipped the widen and the narrow in ONE statement: 'Interim Assistant Coach' was dropped at zero rows in the same transaction that added the four new values, while the picker that still offered it sat unpushed for hours. Production spent that window offering a role the constraint refused — the EXACT gmail-partials shape chunk 5a existed to delete, recreated by sequencing rather than by drift. THE STRUCTURAL CAUSE WAS A WORKFLOW ERROR WORTH NAMING: the UI pass was being treated as the gate that RELEASES a commit, but the reviewer tests PRODUCTION and has no local environment, so every pass was validating code that was not deployed. PUSH, THEN VERIFY, THEN REVERT IF BROKEN — a review of undeployed code confirms nothing and silently keeps a window open.
- A PATH READ OUT OF PROSE IS NOT A ROUTE. /settings/gmail-partials was reported as a live production review queue, broken and then repaired. IT DOES NOT EXIST: the UI was deliberately retired on 2026-08-12 (0 partials ever recorded; only the backend kept), /api/gmail-partials has NO CALLERS anywhere, and the path came from a Section 9 paragraph describing the 2026-04-23 state — CONTRADICTED BY A RECENT CHANGES ROW IN THE SAME FILE, which was not read. The claim compounded: an endpoint's 500-on-error was then described as "returning an empty list", a rendering behaviour invented for a page that does not exist. THIS DOCUMENT DESCRIBES SEVERAL DIFFERENT DATES AT ONCE — Section 9 is layered by stretch, and an older paragraph is not superseded in place. Before asserting a surface exists, list the route directory and grep for a caller; the doc tells you what was BUILT, never what is LIVE.
- A RESOLVE THAT DOES NOT MATCH STILL REPORTS SUCCESS. During an E2 push split a rebase conflict was "resolved" by a script whose regex required content on the HEAD side; that conflict had an EMPTY head side, so the pattern silently did not match — and git rebase --continue proceeded anyway, because the files had already been git add-ed. The commit landed with conflict markers inside CLAUDE_CONTEXT.md and the context generator, and NOTHING reported a problem: the resolve script's failure scrolled past in a traceback, and git reported a clean rebase. Same shape as the killed cron, the discarded PostgREST error, the empty family-state map and the gate's own count — THE TOOL REPORTED PROGRESS AND HAD DONE NOTHING. It was caught by grepping the files for markers before pushing, so that grep now lives in scripts/check-conflict-markers.mjs and runs FIRST in prebuild rather than in anyone's habit. THE GENERAL FORM: when a fix-up step and a continue step are separate commands, the continue does not verify the fix-up. Check the artefact between them.
- EVERY GUARD IN THIS PROJECT STOPPED AT src/, AND THREE OF THEM WERE WRONG TO. The typechecker (scripts excluded from tsconfig), the tenancy fence (scans src/ for raw service clients), and the embed-contract gate (walked src/ only) each enforced a real rule over two thirds of the repo while reading as though it covered all of it. scripts/backfill-camp-extraction.ts held all three holes at once: it called shouldSkipProposal with NO familyId — the parameter made REQUIRED so the compiler would find every call site — it built a RAW service client and read contact_log and schools across every family, and it carried a bare schools!inner embed that had been ambiguous since the email boundary added schools.origin_contact_log_id. Three guards, three misses, one file. CORRECTED 2026-08-22 — this line used to end "all three now scan scripts/" and that was wrong about one of them. TWO were extended: the typechecker (tsconfig.scripts.json) and the embed-contract gate (SRC_DIRS is src plus scripts). THE TENANCY FENCE STILL WALKS src/ ONLY, because ten scripts legitimately hold service role and extending it needs an allowlist designed first. See What the three guards do NOT check.
- A GUARD'S EXCLUSIONS ARE PART OF ITS CLAIM, and its heuristics are too. The embed gate skipped any select matching /^(count|id)\\b/ without an alias — meant to ignore count shorthands, it actually dropped every select that merely STARTED with "id," and used an UNALIASED embed. FIVE contracts were being silently skipped. ONE was a live production path: camp-extractor's extractAndProposeCamps, fired from gmail-sync and the SendGrid inbound webhook, returning PGRST201 at runtime since the FK landed. The other broken embed was api/gmail-partials, which has NO CALLERS AT ALL — its UI was deliberately retired on 2026-08-12 (0 partials ever recorded) and only the backend was kept. It was reported here as a production review queue on the strength of a path read out of Section 9 PROSE describing the 2026-04-23 state, never checked against the filesystem, and contradicted by a Recent Changes row in this same file. The gate reported "12 embeds verified" the whole time — a true statement that answered a narrower question than anyone reading it believed. Its matcher also allowed only ONE !hint, so schools!constraint!inner was invisible to it. WHEN A GATE REPORTS A COUNT, ASK WHAT IT DECLINED TO COUNT.
- BOTH OF TODAY'S HOLES WERE FOUND BY SWEEPS PROMPTED BY SOMETHING UNRELATED — the SQL-grant exposure on get_voice_references came out of a sweep triggered by an over-grant on fk_graph, and the three scripts/ holes came out of turning on a typechecker for an unrelated reason. THAT IS THE SENTENCE THAT CONNECTS THEM: a guard covers the place you were looking. Neither hole was found by a guard, by a review, or by anyone asking the right question — they were found because something adjacent forced a look. Budget for adjacent sweeps; they are not overhead, they are the only thing that has found this class of defect.
...
- CLEARED (2026-08-19): per-family inbound routing shipped — the sendgrid webhook resolves the family from the envelope. STILL Almond-hardcoded: TWO content crons, gmail-sync and summary-refresh (per-family Gmail OAuth is the blocker). CORRECTED 2026-08-22 — this line said all four; camp-discovery and coach-roster-sync were unpinned to the union of families on 2026-08-19.
...
|---|---|---|
| 2026-08-22 | CONTEXT BROUGHT CURRENT BEFORE G, the irreversible sitting — a fresh session on a different model boots from this doc and runs G from it. TWO NEW SECTION 9 SUBSECTIONS. E2 — CATALOG COACHES states the chunk ledger precisely: 2, 3, 5a, 5b and F are shipped and verified, all of them additive or code-only, and G IS THE ONLY IRREVERSIBLE STEP because it drops family_id, school_id, is_primary and notes and no code revert brings those back. G's runsheet is recorded with the correction that THE PRE-IMAGES ARE CAPTURED IN G'S OWN SITTING with counts asserted in-transaction, never ahead of it — coach-roster-sync runs Sunday and Wednesday, so a snapshot taken at F leaves everything the scraper adds in between with no rollback, which is the lesson E1.5 already recorded for camps and E2 initially ignored. Two G design items are new: THE COACH-REMOVAL STORY (rejecting a coach_added proposal leaves the row it created; the only removal path today is SQL, and Tim Howard, a test coach hand-created at Bowdoin on 08-22, becomes every family's staff the moment coaches goes catalog) and THE FENCE'S CONSTRUCTION-NOT-USE GAP (after G, catalogAdmin passes coach writes through unscoped and the fence only ever asked where the client came from — the coach-write inventory that resolves it HAS NOT BEEN DONE and G must not start treating it as answered). WOMEN'S SOCCER SUPPORT, decided today and previously existing nowhere but a chat: discovery_schools stays the INSTITUTION while a PROGRAM dimension goes on coaches, camps and coach-page URLs, because staff pages, rosters and camps are per-program in the real world. G'S PERMANENT KEYS INCLUDE PROGRAM — that is why this lands now, one migration before it becomes a re-migration of the table G just re-pointed. H's scan set becomes school-and-program pairs and H is built program-aware from the start; the player's sport scopes everything a family sees; women's URL discovery and scraping are PHASE 1.5 after H; and alpha wave two is gated on women's COVERAGE BEING REAL, not on the column existing — inviting on schema is inviting a family to an empty product. FIVE CORRECTIONS, all things the doc asserted that were no longer true. (1) THE CRON PINS: Tenancy Architecture and the Productization list both said all four content crons run familyAdmin(ALMOND_FAMILY_ID) and that the camp-discovery pin must not be removed, while the SAME SECTION'S other item said the pin came off on 08-19 — the doc contradicted itself and the stale half read as current. Verified in code: camp-discovery and coach-roster-sync scan the union of families; gmail-sync and summary-refresh stay pinned to Almond's Gmail OAuth. (2) THE TENANCY FENCE DOES NOT SCAN scripts/ — the binding rule claimed all three guards were extended, and only two were; the file calls walk on src and nothing else. Recorded with the reason it is not a one-line fix: ten scripts legitimately hold service role, so it needs an allowlist designed first. (3) THE CAMPS POLICY STATE was recorded only as a code change, so a reader could conclude the ALL/true/true hole was still open. It is closed — policy swapped, four checks passed — and the three-part order is now written down as the precedent G follows: endpoints, UI gate in the same commit, policy last. (4) THE FUNCTION-GRANT PRESCRIPTION in Section 8 still told the reader to add an internal caller assertion as the second lock, which was superseded the same day by PREFER SECURITY INVOKER — an assertion is a lock a re-emission can drop, RLS is not. That paragraph was also sitting inside the Key paths bullet list; it now sits outside it. (5) assertFixtureShape's audit finding is marked CLOSED where the harness section describes it, because the changelog row reporting it still reads as an open gap. ALSO: Section 4's coaches block was three chunks out of date (no family_id, no is_active, no discovery_school_id, no role_source, no endowed_title, and the pre-5b role vocabulary) and coach_family_state was absent from Section 4 entirely — both now current, both stating HOW the column list was obtained since it is not a catalog dump. And two literal BACKSPACE control characters were removed from the doc, where a \\b in a regex description had been written as an escape rather than as text. | Docs |
| 2026-08-21 | E2 CHUNK 5b — THE EIGHT VALUES, THE ABBREVIATION FIX, AND A PICKER LEAK THE INVENTORY HAD ALREADY NAMED. COACH_ROLES is now Head / Interim Head / Associate Head / Assistant / Goalkeeper / Director of Operations / Volunteer Assistant / Other, matching the widened CHECK; Interim Assistant Coach dropped at zero rows. THE ABBREVIATION BUG WAS LIVE AND ON THE FIELD THAT DECIDES WHO A FAMILY EMAILS: normalizeRole tested includes('associate'), so "Assoc. Head Coach" missed it, fell through to includes('head') and normalised to HEAD COACH — and role === 'Head Coach' is exactly what auto-designates the primary contact in all three insert paths, so an abbreviated roster listing could hand the designation to an associate over the actual head coach. "Asst. Coach" fell to Other the same way. Now punctuation-stripped and prefix-matched, most-specific-first (a Goalkeeper Coach also contains "coach"; a Volunteer Assistant also contains "assistant"), EXERCISED over 20 abbreviation forms rather than reviewed — 20/20. THE PICKER DEFECT, found by Randy in the UI: PrepForCallModal still offered a HIDDEN coach. The earlier edit had wrapped the DEFAULT selection in selectableCoaches and left the dropdown mapping over the unfiltered list — the inventory named the site and the edit reached half of it, which is the surface rule and the grep rule pointing at one failure. UploadPrepDocModal had the IDENTICAL shape (two of four sites, not one), and SchoolModal's draft-email path was relying on an upstream filter rather than defending itself. All four now read from one filtered list, and were RE-EXERCISED with a hidden coach through a probe that also asserts each site's source line still contains the expression under test, so a transcription slip in the probe fails loudly instead of passing. A WORSE DEFECT SURFACED WHILE FIXING IT: fetchCoachFamilyState discarded its error, so a failed read returned an EMPTY MAP — and an empty map means NOBODY IS HIDDEN. A failed read rendered identically to a family that had hidden no one, and it failed in the LEAKING direction: every hidden coach reappearing in every picker, silently. That is fail-closed-on-absence in its fourth costume. It now THROWS, and useCoaches catches, surfaces the error and leaves the previous lists intact rather than rendering an un-hidden roster. THE OTHER HALF VERIFIED AS THE ASSEMBLED ARTIFACT, not the composition: a generated call-prep prompt still NAMES the hidden coach, with role and the HIDDEN marker — "- Hidden Person (Goalkeeper Coach, HIDDEN by the family — do not propose contacting, g@probe.edu)" — so the filtering is not over-applied in the generator direction. RECLASSIFICATION IS BLOCKED ON EVIDENCE THE DATABASE DOES NOT HOLD: normalizeRole is LOSSY and nothing persists the raw roster string (coach_added.details carries name/role/email/phone, where role is already normalised), so 7 of the 8 'Other' rows cannot be classified from stored data at all. All 8 sit at Nope-tier schools, none in the scan set. Only Sam Biek is answerable, from his own note. | Feature |
| 2026-08-22 | E2 CHUNK F — PROVENANCE THE PIPELINE HAD BEEN THROWING AWAY. F's SQL added coaches.discovery_school_id (backfilled 254 of 254, zero nulls), role_source and endowed_title; this is the code half. THE POINT IS ECHO OVER DERIVE, APPLIED TO OUR OWN PIPELINE: normalizeRole is LOSSY, and when the role vocabulary widened, 7 of the 8 'Other' rows could not be reclassified from stored data AT ALL because the only evidence — the raw string the page showed — had been discarded at extraction. ScrapedCoach now carries roleSource alongside the normalised role; the scraper persists both role_source and endowed_title on INSERT, the raw string travels in coach_changes.details so the apply path persists the same value rather than re-deriving it, and a PROVENANCE REFRESH updates matched coaches whose stored value has drifted. The refresh writes DIRECTLY with no proposal, deliberately: role_source is a record of what the page said, not a claim about the world, so a review queue for "the page still reads the same" would be noise — and it writes ONLY ON DRIFT, so a scrape that changes nothing issues no writes. endowed_title closes a three-month gap: the extractor has always modelled it as a first-class field (correctly reading "Bobby Clark Head Coach of Men's Soccer" as role='Head Coach' plus endowed_title='Bobby Clark') and applyChanges then DROPPED it because there was no column to write to. unhideCoach now DELETES the coach_family_state row when notes is null rather than writing hidden_at=null into an empty one — the table is SPARSE BY DESIGN, absence is the default, and toggling hide repeatedly used to leave a row asserting nothing each time; a row carrying notes is updated, not deleted. Five branches exercised. THE PREIMAGES MOVED TO G, correcting this runsheet: a snapshot captured at F against a re-point run later leaves everything coach-roster-sync adds in between with no rollback, and it runs Sunday and Wednesday — the same lesson E1.5 recorded and this chunk had initially ignored. F is purely additive; rollback is three dropped columns. | Feature |
| 2026-08-22 | FOUR SILENT FAILURES FROM THE FIRST PASS AGAINST LIVE CODE, and the pattern behind them is bigger than any of them. TWO WERE MINE, WRITTEN HOURS EARLIER TO CLOSE A SECURITY HOLE, and PRODUCTION RUNTIME LOGS name them exactly: /api/camps/[id] threw "'camp_family_status' is a family table — use familyAdmin(familyId), not catalogAdmin/rawService" three times, and /api/camps/[id]/attendees threw the same for 'schools' once. I had used rawService() for cross-family reads while writing a comment claiming that was appropriate; THE WRAPPER EXPLICITLY REFUSES IT, and the tripwire fired precisely as designed. An admin route has no family scope, so it reaches a family table only by taking each family's scope in turn (listFamilies + familyAdmin, the cron-scan-set pattern) or not at all — the attendee route now takes the resolved catalog id FROM THE CLIENT, which already holds the school, instead of reading schools server-side. THE THIRD IS THE OLDEST AND THE WORST: clicking a camp status pill as a second family did nothing, because updateFamilyStatus was a bare UPDATE ... WHERE camp_id = $1. Every camp_family_status row in the database was seeded by createCamp for the family that created the camp — Almond 76 of 76, Testerson ZERO — so the INSERT path had never been exercised by anybody, and an UPDATE matching no rows returns NO ERROR. camp_family_status is the per-family layer of a shared table: ABSENCE IS ITS NORMAL STARTING STATE. Now insert-or-update, family_id from the column DEFAULT. THE FOURTH did not reproduce: driving the live UI as Randy on Bowdoin, the staff section renders "Hidden by you (1)" and the prep-for-call dropdown contains exactly three options with the hidden GK coach absent. The deploy was READY at 20:53Z and the coach was hidden at 21:02Z, so the code was live — but an open SPA session keeps its old bundle until a reload, which is the likeliest explanation and is a REPORTING HAZARD OF THE NEW PUSH-THEN-VERIFY WORKFLOW worth naming. THE PATTERN: all four failed SILENTLY. Every camp mutation returned an error string that every call site discarded. All six now report through one place — useCamps exposes mutationError, each wrapper sets it, and CampDetailClient renders it — because a mutation that cannot report its own failure is what this project keeps paying for, and this was its fifth appearance in a week. | Bug fix |
| 2026-08-22 | CAMPS WRITES MOVE SERVER-SIDE, AND THE UI GATES IN THE SAME COMMIT. camps and camp_school_attendees carried ALL/true/true RLS — any signed-in user could insert, update or delete them from the browser. E1.5 recorded that policy as deliberate-and-temporary on the reasoning that camps were admin-edited AT THE ROUTE LAYER, but the route layer is not a control when the browser writes the table directly. WORSE THAN THE CONSOLE: CampDetailClient's isAdmin guard covered ONLY the host field, so name, dates, location, cost, deadline and notes were editable by any signed-in family THROUGH THE UI — Testerson could edit Almond's camp by clicking. FIVE ENDPOINTS, all requireAdmin + catalogAdmin: POST /api/camps, PATCH and DELETE /api/camps/[id], POST and DELETE /api/camps/[id]/attendees. The client helpers call them; changing their signatures made the COMPILER find all four useRealtimeData call sites. THE SPLIT E1.5 DESIGNED IS PRESERVED EXACTLY: camp_family_status stays a FAMILY table written from the browser, so status, notes, the action-item sync and the created camp's own 'interested' seed remain RLS-scoped to the caller. createCamp and updateCamp are deliberately TWO HALVES, TWO CLIENTS for that reason. UI GATED IN THE SAME COMMIT rather than left as a follow-up, because a button that always fails then explains itself in an error is the shape this project spent the week removing (the unreachable accept form, the nested add-a-school button, the fabricated Interested chip). For a non-admin family every camp field renders READ-ONLY — camp facts ARE shared truth, so seeing them without editing them is honest and no error copy is needed. Gated: the camp name, all seven detail rows, the host row's affordance (its handler was already guarded, its click target was not), attendee add and remove, the delete section (hidden entirely), and both AddCampModal entry points with isAdmin threaded from calendar/page.tsx and schools/[id]/page.tsx. A LATENT CROSS-FAMILY BUG FIXED IN PASSING: camp_family_status cascades when a camp is deleted but the action_item it points at does not, and the browser version cleaned up only the CALLER's reminder because RLS scoped it — an admin deleting a shared camp would have stranded every other family's. The endpoint sweeps all families. Latent not live: 0 of 75 rows carry an action_item_id and no camp is tracked by more than one family, which is exactly why now. AND ONE I INTRODUCED AND CAUGHT: replacing createCamp wholesale dropped its camp_family_status seed and deadline sync, which would have made every new camp arrive untracked with no reminder — found by diffing my own removal against the original rather than trusting the replacement. | Bug fix |
| 2026-08-22 | CONFLICT-MARKER FENCE, and a read-only audit of the other two guards. THE INCIDENT: during the E2 push split a rebase conflict was resolved by a script whose regex required content on the HEAD side; this conflict had an EMPTY head side, the pattern silently did not match, and git rebase --continue proceeded anyway because the files were already git add-ed. Markers landed inside CLAUDE_CONTEXT.md and the generator with nothing reporting a problem — the resolve script's traceback scrolled past and git reported a clean rebase. Caught by grepping for markers before pushing; that grep is now scripts/check-conflict-markers.mjs, running FIRST in prebuild. Proven to bite (marker present → exit 1). It searches only the open and close markers, never the divider, because the divider is also a Markdown h1 underline — an exclusion stated in the file, per the rule that a guard's exclusions are part of its claim. THE AUDIT, reported not changed. TENANCY FENCE: scans src/ only (scripts/ construct raw clients freely, which is how backfill-camp-extraction did); matches .ts/.tsx only, so five .jsx files under design-preview are invisible (none touch Supabase today, verified); detects exactly two patterns — the literal SUPABASE_SERVICE_ROLE_KEY and a NAMED import of createClient from @supabase/supabase-js — so a namespace import, a require, or a dynamic import would all pass; and it checks CONSTRUCTION, never USE, so nothing enforces the doc's claim that rawService is storage/auth/rpc plumbing only. It also cannot see .rpc() calls at all, which is the exact gap the get_voice_references exposure lived in. HARNESS GATE: it is not a gate — it is a manual command, in no prebuild and no CI, costing an Anthropic call per run, and it cannot run at all on a fresh machine because its fixtures are gitignored. validateCampDoc checks SHAPE only; diffDocs is INFORMATIONAL and prints changes without failing, so a semantic regression exits 0. And assertFixtureShape — written days earlier in this same sequence — validates ONLY the coaches array, leaving contactLog, offers, player, extraction, preferences and schoolList able to go stale exactly as coaches did: a guard scoped to the thing that had just broken, reading as though it validated the fixture. | Bug fix |
| 2026-08-22 | THREE GUARDS STOPPED AT src/, AND ONE FILE HELD ALL THREE HOLES. Fixing the shouldSkipProposal call site turned into the sweep that found them. scripts/backfill-camp-extraction.ts (a) called shouldSkipProposal with NO familyId — required precisely so the compiler would find every caller, and camp_proposals is SHARED so a family-less suppression suppresses for everyone; (b) built a RAW service client and read contact_log AND schools across every family, so one family's email was resolved against another family's school list for attendee candidates; and (c) carried a bare schools!inner embed, ambiguous since the email boundary added schools.origin_contact_log_id. THE FAMILYID ANSWER: the script's unit of work is a contact_log row joined to its school, and BOTH are family tables, so every row is family-unambiguous — but the honest fix was not to pass school.family_id, it was to iterate families through familyAdmin like camp-discovery does, which makes familyId come from the loop and scopes every read by construction. It also had NO env loader and was not runnable as checked in. THE EMBED GATE'S OWN HEURISTIC WAS WORSE THAN THE BUG IT MISSED: it skipped any select matching /^(count|id)\\b/ without an alias, which dropped every select merely STARTING with "id," that used an UNALIASED embed — FIVE contracts silently unverified, ONE of them a live production path (camp-extractor's extractAndProposeCamps, fired from gmail-sync and the SendGrid webhook) returning PGRST201 since the FK landed; the other, api/gmail-partials, has NO CALLERS — its UI was retired on 2026-08-12 and only the backend kept, a fact recorded in this very table and not checked, while the gate reported "12 embeds verified" — a true statement answering a narrower question than any reader believed. Its matcher also allowed only one !hint, so schools!constraint!inner was invisible. Now 17 contracts verified, all three named. All three guards scan scripts/. Section 9 gains: every guard stopped at src/; a guard's exclusions AND heuristics are part of its claim, so when a gate reports a count ask what it declined to count; and both of today's holes were found by sweeps prompted by something unrelated — a guard covers the place you were looking. | Bug fix |
| 2026-08-22 | scripts/ JOINS THE TYPECHECKER, AND IT IMMEDIATELY FOUND THE THING IT WAS BUILT TO FIND. tsconfig.scripts.json now type-checks scripts/ + src/ together and runs FIRST in prebuild, closing the blind spot that produced three instances in one week: the ninth CoachRole union, a writer in backfill-coaches.ts, and test-prompt-render calling get_voice_references with no arguments. THE FIND THAT JUSTIFIES THE CHUNK: scripts/backfill-camp-extraction.ts called shouldSkipProposal WITHOUT familyId — the parameter chunk I made REQUIRED specifically so the compiler would find every call site. It could not find that one. camp_proposals is a SHARED catalog table, so a suppression decided without a family suppresses for everyone; the script also builds a RAW service client (scripts/ is outside the tenancy fence too, which is the fence's second blind spot after SQL function grants) and had never been updated for tenancy. familyId and hostDiscoverySchoolId now come from the school row. TWO MORE, both real: backfill-message-coverage cast a PostgREST embed ARRAY straight to an object, so school?.name was always undefined and every row logged "Unknown"; and scripts/test-prompt-render.ts was DELETED rather than repaired — it bootstraps a migration that already ran, calls a function whose signature changed, and both READS AND WRITES player_profile, which has been frozen since C6 and is dropped at C7, so there is no correct version of it to keep. 21 errors at the start, 19 of them in that one file. THE ES5 NOISE WAS CONFIG, NOT CODE: the base tsconfig targets es5 for Next's browser output, which made every Map/Set iteration in a Node script a TS2802; the scripts config targets es2022 and 9 errors vanished without touching a line. GATE PROVEN TO BITE, not assumed: removing familyId again reproduces the error and fails the build with exit 1; restoring it returns to 0. | Refactor |
...
async function main() {
  ...
  for (const tier of ['A', 'B', 'C']) {
    ...
    pipelineLines.push(`### ${tierLabel} (${tierSchools.length} ${tierSchools.length === 1 ? 'school' : 'schools'})`)
    pipelineLines.push('')
