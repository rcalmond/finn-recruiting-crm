...
- THE UNION CRONS EXCEED VERCEL'S 300s CEILING AT TWO FAMILIES. NAMED CONSTRAINT, NOT A NOTE: camp-discovery cannot run to completion in production as written. Measured 2026-08-19 — 20 (family, school) pairs took 371 SECONDS. The scheduled runs were ALREADY failing before the union landed, and far more broadly than a first look suggested: across its ENTIRE history camp-discovery has 16 runs and only 3 successes — 13 were killed, every Saturday from 2026-05-09 through 2026-08-01. It began completing only on 08-08. coach-roster-sync has 11 killed runs of 32, though none since 2026-07-12. Cost is close to linear at roughly 18 seconds per (family, school) pair: 100s for 7, 182s for 10, 371s for 20 — so the 300s ceiling is reached at about 16 pairs, and the current scan set is 20. A killed function never writes its completion row, so a dead run looks like a run still in progress forever, in a table nobody reads. coach-roster-sync is fine TODAY at 32 to 70 seconds, and only because no second-family school has a coach_page_url yet — it scales exactly the same way. WHAT THIS MEANS RIGHT NOW: the next scheduled camp-discovery will be killed partway through, and because the work is a flat loop over pairs, the families late in the loop are the ones that silently never get scanned. THE SHAPE OF THE FIX, unbuilt and deliberately not chosen here: chunk by family so each invocation handles one family and the schedule fans out; or move the per-pair work onto a queue (Vercel Queues) with the cron as the enqueuer; or a background job with its own runtime. All three make the unit of work small and resumable, which is the actual requirement — the current design has no way to make progress across a kill. THIS IS A PREREQUISITE FOR ANY FAMILY COUNT ABOVE A HANDFUL, and it must be fixed before onboarding, not discovered when a family's schools quietly stop being scraped.
